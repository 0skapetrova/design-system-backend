...
### 3.1 
Пользователь начинает регистрацию с ввода номера телефона. Система отправляет SMS-код и проверяет, не существует ли уже аккаунт с этим номером. После успешного подтверждения телефона пользователь заполняет данные учётной записи (уникальный username, display name (необязательное поле) и пароль). После создания аккаунта backend возвращает информацию о пользователе, и клиент может перейти к рабочему состоянию приложения.
